...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Initialize EmailJS
      emailjs.init('v8fI6BA4Ha8tkiAY2')
      ...
      const result = await emailjs.send(
        ...
        'template_lmgm5l9',
        {
          to_email: 'growthhiveksa@gmail.com',
          from_email: formData.email,
          message: formData.message,
          to_name: 'Growth Hive KSA',
  ...
  const socialLinks = [
    { 
      icon: 'instagram', 
      name: 'Instagram', 
      link: 'https://www.instagram.com/growthhiveksa?igsh=MXZlNW10cGdla240bw==',
      color: 'hover:bg-gradient-to-r hover:from-purple-500 hover:to-pink-500'
    },
    { 
      icon: 'facebook', 
      name: 'Facebook', 
      link: 'https://www.facebook.com/share/16No8Eb8eb/',
              ...
              <div className="flex space-x-4">
                {socialLinks.map((social, index) => (
                  <motion.a
                    ...
                    className={`w-14 h-14 bg-white/10 rounded-xl flex items-center justify-center transition-all duration-300 border border-white/20 ${social.color} hover:scale-110 hover:shadow-lg`}
                    ...
                    whileTap={{ scale: 0.9 }}
                  >
                    <SocialIcons type={social.icon} size="w-7 h-7" />
                  </motion.a>
                ))}
              </div>
            </div>
